...
using msg = System.Windows.Forms;
...
namespace Ametrano.Logica
{
    class Alumnos_Controlador
    {
        ...
        public bool insertarAlumno(DatosAlumno datos, int id_grupo, out int exitCode)
        {
            ...
            //QUERY PRINCIPAL 
            string queryBuscar = "select a.cedula_alumno, b.id_grupo from alumno a join asiste b on b.cedula_alumno = a.cedula_alumno and b.cedula_alumno = " + datos.getDatosPersona()["cedula_alumno"] + " group by(a.cedula_alumno)";
            MySqlDataAdapter adapter_buscarAlumno = objetoConexion.consultarDatos(queryBuscar);
            DataTable dt_buscarAlumno = new DataTable();
            adapter_buscarAlumno.Fill(dt_buscarAlumno);
            string ci = "", id_grupo_busqueda = "";
            if (dt_buscarAlumno.Rows.Count > 0)
            {
                ci = dt_buscarAlumno.Rows[0][0].ToString();
                id_grupo_busqueda = dt_buscarAlumno.Rows[0][1].ToString();
            }
            

            int updateAlumnoRowsAfected = 0;
            int insertAlumnoRowsAfected = 0;
            int insertAsisteRowsAfected = 0;

            if (ci.Equals(datos.getDatosPersona()["cedula_alumno"]))
            {
                msg.MessageBox.Show("¡Alumno ya existente!\nSe actualizaran los datos", "Informacion", msg.MessageBoxButtons.OK, msg.MessageBoxIcon.Exclamation);
                string queryUpdate = "UPDATE alumno SET cedula_alumno='{0}', nombre1='{1}', nombre2='{2}', apellido1='{3}', apellido2='{4}', fecha_nac='{5}', edad={6}, sexo='{7}', estado_civil='{8}', nivel_educativo='{9}', ultimo_anio_aprobado='{10}', telefono_fijo='{11}', celular='{12}', email='{13}', departamento='{14}', calle='{15}', numero_puerta='{16}', apartamento='{17}', referencia='{18}', localidad='{19}', estado='{20}', trabajo_alguna_vez={21}, trabaja_actualmente={22}, tiempo_sin_trabajo='{23}', horas_trabajo='{24}', ingreso_mensual='{25}', cobertura_salud='{26}', jefe_hogar={27}, cant_hijos={28}, usa_internet={29}, facil_acceso_internet={30}, medio_acceso_internet='{31}', cant_personas_cargo_17={32}, cant_personas_cargo_18_59={33},cant_personas_cargo_60={34}, persona_tiene_discapacidad={35},cuenta_con_apoyo={36}, carga_semanal_cuidado='{37}',trabajo_anteriormente_cuidando={38}, experiencia_instituciones_cuidado='{39}', monto_viatico_por_dia={40} WHERE cedula_alumno='{41}'";
                queryUpdate = string.Format(queryUpdate, datos.getDatosPersona()["cedula_alumno"], datos.getDatosPersona()["nombre1"], datos.getDatosPersona()["nombre2"], datos.getDatosPersona()["apellido1"], datos.getDatosPersona()["apellido2"], datos.getDatosPersona()["fecha_nac"], datos.getDatosPersona()["edad"], datos.getDatosPersona()["sexo"], datos.getDatosPersona()["estado_civil"], datos.getDatosPersona()["formacion_nivel"], datos.getDatosPersona()["formacion_ultimo_año_aprovado"], datos.getDatosPersona()["contacto_telefono"], datos.getDatosPersona()["contacto_celular"], datos.getDatosPersona()["contacto_email"], datos.getDatosPersona()["direccion_departamento"], datos.getDatosPersona()["direccion_calle"], datos.getDatosPersona()["direccion_numero_puerta"], datos.getDatosPersona()["direccion_apartamento"], datos.getDatosPersona()["direccion_referencia"], datos.getDatosPersona()["direccion_localidad"], datos.getDatosPersona()["curso_estado"], datos.getDatosPersona()["trabajo_trabajo_alguna_vez"], datos.getDatosPersona()["trabajo_trabaja_actualmente"], datos.getDatosPersona()["trabajo_tiempo_desempleado"], datos.getDatosPersona()["trabajo_horas_jornada"], datos.getDatosPersona()["trabajo_ingreso_mensual"], datos.getDatosPersona()["cobertura_salud"], datos.getDatosPersona()["hogar_jefe"], datos.getDatosPersona()["hogar_cantidad_hijos"], datos.getDatosPersona()["internet_usa_internet"], datos.getDatosPersona()["internet_facil_acceso"], datos.getDatosPersona()["internet_medio_acceso"], datos.getDatosPersona()["personas_cargo_0_17"], datos.getDatosPersona()["personas_cargo_18_59"], datos.getDatosPersona()["personas_cargo_60_mas"], datos.getDatosPersona()["personas_cargo_con_discapacidad"], datos.getDatosPersona()["personas_cargo_cuenta_con_apoyo"], datos.getDatosPersona()["personas_cargo_carga_semanal_cuidado"], datos.getDatosPersona()["personas_cargo_trabajo_cuidando"], datos.getDatosPersona()["personas_cargo_experiencia_instituciones_cuidado"], datos.getDatosPersona()["curso_monto_viatico"], ci);
                updateAlumnoRowsAfected = objetoConexion.sqlInsertUpdate(queryUpdate);
                exitCode = 2;
            }else
            {
               ...
               "VALUES(" + datosPersonales + datosCurso + datosFormacionAcademica + datosDireccion + datosContacto + datosCoberura + datosHogar + datosTrabajo + datosPersonasCargo + datosAccesoInternet + ");";
                insertAlumnoRowsAfected = objetoConexion.sqlInsertUpdate(query);
                exitCode = 1;
            }

            if (!id_grupo_busqueda.Equals(id_grupo + ""))
            {
                if(insertAlumnoRowsAfected>0 || updateAlumnoRowsAfected > 0)
                {
                    string queryInsertAsiste = "INSERT INTO asiste(nombre_curso, id_grupo, cedula_alumno, nombre_materia, fecha, asistencia) VALUES('{0}',{1},'{2}','{3}','{4}',{5})";
                    queryInsertAsiste = string.Format(queryInsertAsiste, datos.getDatosPersona()["curso_alumno"], id_grupo, datos.getDatosPersona()["cedula_alumno"], "nuevo", "0001-01-01", 0);
                    insertAsisteRowsAfected = objetoConexion.sqlInsertUpdate(queryInsertAsiste);
                }
            }

            bool exito = false;
            if((insertAlumnoRowsAfected > 0 && insertAsisteRowsAfected > 0)||(updateAlumnoRowsAfected > 0)||(updateAlumnoRowsAfected>0 && insertAsisteRowsAfected > 0))
            {
                exito = true;
            }
        ...
        public string MostrarPeriodoAlumno(string cedula, string nombre_curso, string turno)
        {
            ...
            string query = "SET lc_time_names=es_ES;SELECT CONCAT('Numero: ',grupo.serial,' - ',monthname(fecha_inicio),' ',year(fecha_inicio),' - ',monthname(fecha_fin),' ',year(fecha_fin)) from grupo WHERE id_grupo='" + id_grupo + "'";
